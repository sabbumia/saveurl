    ...
    try {
      ...
      if (response.ok) {
        ...
        const uniqueCategories = [
          ...new Set(data.links.map((link: PublicLink) => link.category)),
        ];
        const uniqueSources = [
          ...new Set(data.links.map((link: PublicLink) => link.source)),
        ];
        const uniqueContributors = [
          ...new Set(data.links.map((link: PublicLink) => link.userEmail)),
        ];

        const tags = data.links
          .filter((link: PublicLink) => link.tags)
          .flatMap((link: PublicLink) =>
            link.tags.split(",").map((t) => t.trim())
          )
        ...
        const recentCount = data.links.filter(
          (link: PublicLink) => new Date(link.createdAt) > last7Days
        ).length;
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 py-8 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        <WaveLoader color="#f038a3" text="Loading public links..." fullPage={true} />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 py-8 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute bottom-1/3 right-1/3 w-96 h-96 bg-purple-500 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse"
          style={{ animationDelay: "1s" }}
        ></div>
        <div
          className="absolute top-1/2 right-1/4 w-96 h-96 bg-cyan-500 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse"
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        {/* Stats Cards */}
        <div
          className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 animate-fadeIn"
        ...
        {/* Main Content Card */}
        <div
          className="bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/10 p-6 sm:p-8 animate-fadeIn"
                  ...
                  <div className={viewMode === "list" ? "flex-1" : ""}>
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span
                          className={`px-3 py-1 rounded-lg text-xs font-bold border ${getCategoryColor(link.category)}`}
                        >
        ...
        {/* Footer Info */}
        <div
          className="mt-8 text-center text-gray-500 text-sm animate-fadeIn"
